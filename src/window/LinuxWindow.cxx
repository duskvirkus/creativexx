...
LinuxWindow::LinuxWindow(std::string title,
                         unsigned int width,
                         unsigned int height)
    : Window(),
      m_data({std::move(title), width, height, nullptr}),
      m_window(nullptr) {
  CREATIVE_INFO("Creating window " + m_data.title +
                " w: " + std::to_string(m_data.width) +
                " h: " + std::to_string(m_data.height))
  ...
  glfwSetScrollCallback(
      m_window, [](GLFWwindow* window, double x_off, double y_off) {
        WindowData& data =
            *static_cast<WindowData*>(glfwGetWindowUserPointer(window));

        event::MouseScrolledEvent event(static_cast<float>(x_off),
